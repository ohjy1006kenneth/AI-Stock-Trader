...
from runtime.common.common import BACKTESTS_DIR, MARKET_DATA_DIR, TRADING_DAYS_PER_YEAR, mean, now_iso, read_json, realized_volatility, rsi, safe_float, sma, stddev, trailing_return, write_json, write_text
...
def main() -> None:
    ...
    if not dates:
        raise SystemExit("No price history available in data/runtime/market/price_snapshot.json")
    ...
    outputs_dir = BACKTESTS_DIR / "outputs"
    outputs_dir.mkdir(parents=True, exist_ok=True)

    with (outputs_dir / "trade_log.csv").open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["date", "ticker", "action", "sleeve", "shares", "price", "notional", "reason_code"])
        writer.writeheader()
        writer.writerows(trade_rows)

    with (outputs_dir / "equity_curve.csv").open("w", newline="") as f:
    ...
    write_json(outputs_dir / "metrics.json", metrics_payload)
    ...
    write_text(
        outputs_dir / "backtest_report.md",
        "# Backtest Report\n\n"
