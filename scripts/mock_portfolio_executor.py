...
from common import LEDGER_DIR, OUTPUTS_DIR, gen_id, latest_price_from_snapshot, now_iso, read_json, validate_required_fields, write_json
...
def main() -> None:
    ...
    price_snapshot = read_json(OUTPUTS_DIR / "price_snapshot.json", {"items": []})
    ...
    for d in decisions:
        ...
        if action == "BUY":
            ...
            portfolio.setdefault("trade_history", []).append({"timestamp": now_iso(), "ticker": ticker, "action": "BUY", "shares": shares, "price": price, "notional": notional, "reason_code": d["reason_code"]})
            logs.append(make_log(execution_id=gen_id("exe"), linked_decision_id=d["decision_id"], timestamp=now_iso(), ticker=ticker,
            ...
            portfolio["trade_history"].append({"timestamp": now_iso(), "ticker": ticker, "action": "SELL", "shares": shares, "price": price, "notional": notional, "reason_code": d["reason_code"], "realized_pnl": realized})
            logs.append(make_log(execution_id=gen_id("exe"), linked_decision_id=d["decision_id"], timestamp=now_iso(), ticker=ticker,
    ...
    write_json(OUTPUTS_DIR / "execution_log.json", {"generated_at": now_iso(), "execution_schema_version": "v1", "items": logs})
