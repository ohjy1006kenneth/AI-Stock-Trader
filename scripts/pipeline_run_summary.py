...
def read_recent_validation_note() -> str:
    path = REPORTS_DIR / "backtest_verdict.md"
    if not path.exists():
...
def read_recent_research_note() -> str:
    path = REPORTS_DIR / "research_update.md"
    if not path.exists():
        return "No research-layer changes affected this run; current formulas were used as-is."
    lines = [line.strip() for line in path.read_text().splitlines() if line.strip()]
    return lines[0] if lines else "Research note present but empty; current formulas were used as-is."
...
def main() -> None:
    ...
    quality = read_json(OUTPUTS_DIR / "qualified_universe.json", {"items": [], "rejected": []})
    rankings = read_json(OUTPUTS_DIR / "alpha_rankings.json", {"items": []})
    sentry = read_json(OUTPUTS_DIR / "sentry_events.json", {"events": []})
    strategist = read_json(OUTPUTS_DIR / "strategist_decisions.json", {"decisions": []})
    execution = read_json(OUTPUTS_DIR / "execution_log.json", {"items": []})
    alerts = read_json(OUTPUTS_DIR / "trade_alerts_latest.json", {"items": []})
    ...
    report_path = REPORTS_DIR / f"daily_summary_{ts[:10]}.md"
    ...
    lines = [
        ...
        f"- Current factor stack was used as documented: ranked momentum, realized volatility, trend bonus, and quality bonus over the refreshed snapshots.",
        f"- Research-layer status: {read_recent_research_note()}",
        "- No ad hoc research change was introduced during this pipeline run unless separately recorded in a research update.",
        "",
    ...
    out_path = REPORTS_DIR / f"pipeline_run_summary_{ts[:10]}.md"
    write_text(out_path, "\n".join(lines) + "\n")
    print("\n".join(lines))
