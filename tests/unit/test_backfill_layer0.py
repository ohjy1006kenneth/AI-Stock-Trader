...
def test_wikipedia_universe_provider_reuses_parsed_tables(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    event = _ChangeEvent(date="2010-01-01", added=frozenset(), removed=frozenset())
    ...
    def fake_parse_change_log(html: str) -> list[_ChangeEvent]:
        calls["events"] += 1
        assert html == "<html></html>"
        return [event]
    ...
    monkeypatch.setattr(backfill_layer0, "_fetch_html", fake_fetch_html)
    monkeypatch.setattr(backfill_layer0, "_parse_current_tickers", fake_parse_current_tickers)
    monkeypatch.setattr(backfill_layer0, "_parse_change_log", fake_parse_change_log)
    monkeypatch.setattr(backfill_layer0, "_reconstruct_at_date", fake_reconstruct_at_date)
