...
from runtime.common.common import EXECUTION_DATA_DIR, LEDGER_DIR, MARKET_DATA_DIR, STRATEGY_DATA_DIR, gen_id, latest_price_from_snapshot, load_execution_config, market_is_open_now, now_iso, read_json, safe_float, validate_required_fields, write_json
...
def sync_ledger_from_broker(portfolio: dict, snapshot: dict[str, Any], execution_mode: str) -> dict:
...
def main() -> None:
    ...
    prior_execution = read_json(EXECUTION_DATA_DIR / "execution_log.json", {"items": []})
    ...
    for d in decisions:
        ...
        if missing:
            reject(logs, d, f"malformed_decision_schema:{','.join(missing)}", cash, cash, None, None, execution_mode=execution_mode)
            continue
        if d.get("action") not in VALID_ACTIONS:
            reject(logs, d, "invalid_action", cash, cash, None, None, execution_mode=execution_mode)
            continue
        if d.get("sleeve") not in VALID_SLEEVES:
            reject(logs, d, "invalid_sleeve", cash, cash, None, None, execution_mode=execution_mode)
            continue
    ...
    if execution_mode == "alpaca_paper":
        ...
        cash = float(safe_float(account.get("cash"), cash) or cash)
        positions = normalize_broker_positions(broker_snapshot.get("positions", []))
    else:
        client = None
        open_orders = []
        broker_positions = {p.get("ticker"): p for p in positions}
    ...
    for d in decisions:
        ...
        if ticker in conflicted:
            reject(logs, d, "conflicting_buy_sell_instructions", cash, cash, position_before, position_before, price, execution_mode=execution_mode)
            continue
        if price is None:
            reject(logs, d, "missing_execution_price", cash, cash, position_before, position_before, price, execution_mode=execution_mode)
            continue
        ...
        if not market_open and not allow_queue:
            reject(logs, d, "market_closed_submission_blocked", cash, cash, position_before, position_before, price, execution_mode=execution_mode)
            continue
        ...
        if action == "BUY":
            if config.get("duplicate_order_prevention", {}).get("check_existing_positions", True) and (position_before is not None or ticker in broker_positions):
                reject(logs, d, "duplicate_existing_position", cash, cash, position_before, position_before, price, execution_mode=execution_mode)
                continue
            if len(positions) >= int(rules["max_total_positions"]):
                reject(logs, d, "max_total_positions_exceeded", cash, cash, None, None, price, execution_mode=execution_mode)
                continue
            if target_shares <= 0:
                reject(logs, d, "non_positive_share_request", cash, cash, None, None, price, execution_mode=execution_mode)
                continue
            notional = round(target_shares * price, 2)
            if notional > cash * (1.0 - float(rules["cash_buffer_rule"])):
                reject(logs, d, "insufficient_cash_buffer", cash, cash, None, None, price, execution_mode=execution_mode)
                continue
            if (notional / max(total_equity, 1.0)) > float(rules["max_position_weight"]):
                reject(logs, d, "position_weight_exceeded", cash, cash, None, None, price, execution_mode=execution_mode)
                continue
            if d["sleeve"] == "CORE" and ((sleeve_value("CORE") + notional) / max(total_equity, 1.0)) > float(rules["max_core_allocation"]):
                reject(logs, d, "core_allocation_exceeded", cash, cash, None, None, price, execution_mode=execution_mode)
                continue
            if d["sleeve"] == "SWING" and ((sleeve_value("SWING") + notional) / max(total_equity, 1.0)) > float(rules["max_swing_allocation"]):
                reject(logs, d, "swing_allocation_exceeded", cash, cash, None, None, price, execution_mode=execution_mode)
                continue
        elif action == "SELL":
            ...
            if position_before is None and ticker not in broker_positions:
                reject(logs, d, "selling_nonexistent_position", cash, cash, None, None, price, execution_mode=execution_mode)
                continue
            if target_shares <= 0:
                target_shares = int(float((broker_positions.get(ticker) or {}).get("shares", 0) or 0))
            if target_shares <= 0:
                reject(logs, d, "non_positive_share_request", cash, cash, position_before, position_before, price, execution_mode=execution_mode)
                continue
        ...
        if execution_mode == "alpaca_paper":
            ...
            # local deterministic fallback retained behind the adapter model
            if action == "BUY":
                notional = round(target_shares * price, 2)
                cash_before = cash
                ...
                pos = {
                    ...
                    "avg_cost": price,
                    "last_price": price,
                    "market_value": round(target_shares * price, 2),
                    "unrealized_pnl": 0.0,
                    "entry_date": now_iso(),
                ...
                portfolio.setdefault("trade_history", []).append({"timestamp": now_iso(), "ticker": ticker, "action": "BUY", "shares": target_shares, "price": price, "notional": notional, "reason_code": d["reason_code"], "sleeve": d["sleeve"]})
                logs.append(append_extra(make_log(
                    execution_id=gen_id("exe"), linked_decision_id=d["decision_id"], timestamp=now_iso(), ticker=ticker,
                ...
                realized = round((price - float((position_before or {}).get("avg_cost", 0.0))) * shares, 2)
                cash_before = cash
                cash = round(cash + notional, 2)
                ...
                portfolio.setdefault("trade_history", []).append({"timestamp": now_iso(), "ticker": ticker, "action": "SELL", "shares": shares, "price": price, "notional": notional, "reason_code": d["reason_code"], "realized_pnl": realized})
                logs.append(append_extra(make_log(
                    execution_id=gen_id("exe"), linked_decision_id=d["decision_id"], timestamp=now_iso(), ticker=ticker,
    ...
    if execution_mode == "alpaca_paper":
        ...
        unrealized = 0.0
        for p in positions:
            latest = latest_price_from_snapshot(price_snapshot, p["ticker"])
            if latest is None:
                continue
            p["last_price"] = latest
            p["market_value"] = round(int(p["shares"]) * latest, 2)
            p["unrealized_pnl"] = round((latest - float(p["avg_cost"])) * int(p["shares"]), 2)
    ...
    print(f"Executor finished: {len(logs)} execution log records (mode={execution_mode})")
